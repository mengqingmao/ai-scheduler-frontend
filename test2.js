...
function computeDailyTotals(daysInMonth, scheduleData) {
    ...
    const morningPositions = nightPositions;
    const totals = Array.from({ length: daysInMonth }, () => 0);

    for (let d = 0; d < daysInMonth; d++) {
        const dayData = data[d] || { dayShift: {}, nightShift: {} };
        let count = 0;
        morningPositions.forEach(pos => {
            if (dayData.dayShift && dayData.dayShift[pos]) count++;
        });
        nightPositions.forEach(pos => {
...
function syncMorningMappings(scheduler) {
    ...
    for (let d = 0; d < days - 1; d++) {
        ...
        nightPositions.forEach(pos => {
            ...
            if (nextData.dayShift[flagKey] && !isFixedSeatOnDay(pos, d + 1)) {
                delete nextData.dayShift[flagKey];
                delete nextData.dayShift[pos];
            }
        });
        ...
        nightPositions.forEach(pos => {
            ...
            if (!person) return;
            const flagKey = `__morning__${pos}`;
            if (isFixedSeatOnDay(pos, d + 1)) return;
            // 写入早班映射
...
class Scheduler {
    ...
    const nightQuotaRemaining = {};
        ...
        // 先定义排班顺序：主班/大夜优先
        const orderedNightPositions = ['主班', '大夜一', '大夜二']
            .concat(this.nightPositions.filter(p => !['主班', '大夜一', '大夜二'].includes(p)));
        ...
        for (let day = startDay; day < days; day++) {
            ...
            orderedNightPositions.forEach(pos => {
                if (dayData.nightShift[pos]) return; // 已固定
                ...
                if (nextIdx < days) {
                    ...
                    if (hasSpecialSeatConflictOnDay(this, nextIdx, pos)) return;
                    if (nextDayData.dayShift && nextDayData.dayShift[pos] && isFixedSeatOnDay(pos, nextIdx + 1)) return; // 次日固定席位才跳过
                }
                ...
                let qualified = Array.from(availableForNight).filter(name => {
                    ...
                    // 规则：若次日该人员已有特殊班次，则前一日不安排其夜班（避免早班冲突）
                ...
                for (let qi = 0; qi < qualified.length; qi++) {
                    ...
                    // 映射次日早班（若可）
                    let mappedToNext = true;
                    if (nextIdx < days) {
                        const nextDayData = this.schedule[nextIdx];
                        if (nextDayData.dayShift && nextDayData.dayShift[pos] && isFixedSeatOnDay(pos, nextIdx + 1)) {
                            mappedToNext = false;
                        }
                        else if (this._isPersonFixedOnDay(cand, nextIdx + 1)) mappedToNext = false;
                        else if (isPersonSpecialOnDay(this, nextIdx, cand)) mappedToNext = false;
                        if (mappedToNext) {
